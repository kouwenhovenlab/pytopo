...
class ATS9360ControllerExt(ATS9360Controller):
    ...
    def pre_acquire(self):
        ...
        self.damaz_time = time.time()

    def post_acquire(self):
        print("Damaz says: ", time.time()-self.damaz_time)
        if self.trigger_src:
            self.enable_trigger(False)

        ret = super().post_acquire()
        print("Damaz also says: ", time.time()-self.damaz_time)
        return ret
...
class AlazarMeasurement(BaseMeasurement):
    ...
    def _setup_channels(self, **kw):
        ...
        for c, n in zip(self.chans, ['A', 'B']):
            c.acquisition_kwargs.update(self.namespace.ats_acq_kwargs)
