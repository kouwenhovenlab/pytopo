...
_qcodes_is_with_delegate_parameter = True
try:
    from qcodes.instrument.parameter import DelegateParameter
except ImportError:
    _qcodes_is_with_delegate_parameter = False


if _qcodes_is_with_delegate_parameter:
    ...
    class ConversionParameter(Parameter):
        ...
        of the source parameter `src_param`. Similarly, when this parameter is
        being "get", the value from the `get()` call to the source parameter
        is taken, then that value is passed through `get_conv` function,
        and the
        result of it is returned.

        The reason for using a `ConversionParameter` instead of the source
        parameter
        directly is to provide all the functionality of the source parameter
        without
        changing the source parameter itself. So, `ConversionParameter`
        wraps the
        source parameter, and at the same time it can have its own
        `name`/`label/`unit`/etc, and also have conversion functions on top
        of the
        `get`/`set` functionality of the source parameter.

        `get_conv` function should always be provided. If `set_conv` is not
        provided,
        then it is not possible to `set` the `ConversionParameter`.
            ...
            for cmd in ('set_cmd', 'get_cmd'):
                if cmd in kw:
                    raise KeyError(f'It is not allowed to set "{cmd}" of a '
                                   f'ConversionParameter because the one of '
                                   f'the '
                                   f'source parameter is supposed to be used '
                                   f'together with get_conv and set_conv '
                                   f'functions.')
